...
public class Controller implements Mediator, BoardGame<Integer>, EventHandler<MouseEvent>  {
	...
	private void checkersSquareGuiHandle(MouseEvent mouseEvent) {
		...
		this.moveCapturePromote(this.getToMovePieceIndex(), targetSquareIndex, this.score);
	 ...
	 */
	@Override
	public OutputModelData<Integer> moveCapturePromote(Integer toMovePieceIndex, Integer targetSquareIndex, int [] score) {
		...
		if (this.model != null) {
			outputModelData  = this.model.moveCapturePromote(toMovePieceCoord, targetSquareCoord, score);
		...
		setToMovePieceIndex(targetSquareIndex);
