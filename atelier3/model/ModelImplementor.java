...
public class ModelImplementor {
	...
	public boolean isPromoteOk(Coord coord){
		boolean isPromoteOk = false;
		PieceModel tempiece = this.findPiece(coord);
		if(tempiece !=null && tempiece instanceof Promotable)
		{
			PawnModel Piece = (PawnModel) tempiece;
			isPromoteOk = Piece.isPromotable();
		}
		return isPromoteOk;
	}

	public void Promote(Coord coord, PieceSquareColor color){
		this.pieces.remove(this.findPiece(coord));
		this.pieces.add(new QueenModel(coord, color));
